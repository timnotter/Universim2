...
int main(int argc, char **argv) {
  ...
  while (isRunning) {
    ...
    updateTime = ((1000000000 * (currTime.tv_sec - prevTime.tv_sec) +
                  ...
                  1000);
    // printf("Handling events and drawing took %d mics, compared to wanted %d
...
void localUpdate(std::mutex *currentlyUpdatingOrDrawingLock,
  ...
  while (*isRunning) {
    ...
    if (loneStarUpdateCounter++ == LONESTAR_BACKOFF_AMOUNT) {
      ...
      for (int i = 0; i < threadNumber - 1; i++) {
        threads.push_back(std::thread(
            updateLoneStarStarsystemMultiThread, &loneStars, i * amount,
            i * amount + amount, (TIMESTEP_LOCAL * LONESTAR_BACKOFF_AMOUNT)));
      }
      threads.push_back(
    ...
    // If it is time, wait until the stellar force calculation has finished and
    // insert it's values. From then on use the new stellar acceleration
    ...
    updateTime = ((1000000000 * (currTime.tv_sec - initialTime.tv_sec) +
                  ...
                  1000);
    // printf("Updating took %d mics, compared to wanted %d mics. Now sleeping
    // %d\n", updateTime, *optimalTimeLocalUpdate,
    // (*optimalTimeLocalUpdate-updateTime));
    if ((*optimalTimeLocalUpdate - updateTime) > 0)
      std::this_thread::sleep_for(
          std::chrono::microseconds(*optimalTimeLocalUpdate - updateTime));
    getTime(&initialTime, 0);
  }
}
...
void stellarUpdate(std::mutex *currentlyUpdatingOrDrawingLock,
  ...
  while (*isRunning) {
    ...
    // Make sure the local update thread has picked up the localUpdateIsReady
    // lock
...
void initialiseStellarObjects(std::vector<StellarObject *> *galaxies,
  ...
  // // this inclinations is from the solar plan to the galactic plane, thus not
  // of interest
  ...
  ADD_PLANET(
      new Planet("Mars", 0.532, 0.107, 1.52368055, 0.0934, 1.85 * PI / 180, 0));
  ...
  ADD_PLANET(
      new Planet("Jupiter", 10.973, 317.8, 5.204, 0.0489, 1.303 * PI / 180, 0));
  ...
  readMoonFile(MOONS_OF_JUPITER_FILE_PATH, galaxies->back()
                                               ...
                                               ->back());
  ADD_PLANET(
      new Planet("Saturn", 8.552, 95.159, 9.5826, 0.0565, 2.485 * PI / 180, 1));
  ...
  ADD_MOON(new Moon("Charon", 606000 / lunarRadius, 1.586e21 / lunarMass,
                    17181000 / distanceEarthMoon, 0.0002, 112.783 * PI / 180));
  ...
  for (int i = 0; i < threadNumber - 1; i++) {
    ...
    threads.push_back(std::thread(spawnStarSystemsMultiThread, galaxies, amount,
                                  currentlyUpdatingOrDrawingLock,
                                  densityFunction));
  }
...
void readMoonFile(std::string fileLocation, StellarObject *parent) {
  ...
  while (!moonFile.eof()) {
    ...
    moonFile >> radius;
    // counter++;
    // If the period is to short, we ignore the moon, for it would be simulated
    // badly anyways
    if (std::abs(period) < 1)
      continue;
    parent->addChild(
        new Moon(name.c_str(), radius / lunarRadius, mass / lunarMass,
                 meanDistance / distanceEarthMoon, eccentricity, inclination));
    moonFile.peek();
    // printf("Read %s, radius: %Lf, mass: %Lf, meanDistance: %Lf, eccentricity:
    // %Lf, inclination: %Lf, period: %Lf\n", name.c_str(), radius/lunarRadius,
    // mass/lunarMass, meanDistance/distanceEarthMoon, eccentricity,
    // inclination, period);
  }
  moonFile.close();
}
