...
#define STANDARD_CAMERA_POSITION                                               \
  -1 * std::min(std::max(2 * centreObject->getRadius(), solarRadius / 5),      \
                astronomicalUnit)

// An object that is to close to be drawn as a simple circle gets assigned a
// CloseObject
...
// Starts threads to update a snapshot of all positions. This is used before
// drawing the canvas
